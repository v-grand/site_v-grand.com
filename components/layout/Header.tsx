                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-between h-20">
                        ...
                        <nav className="hidden md:flex items-center space-x-8">
                            {navLinks.map(link => (
                                <a key={link.name} href="/" className="text-base font-medium text-slate-600 hover:text-sky-500 transition-colors">
                                    {link.name}
                                </a>
                            ))}
                                    ...
                                    <div className="absolute -left-8 mt-4 w-64 rounded-lg shadow-lg bg-white border border-slate-200 ring-1 ring-black ring-opacity-5">
                                        <div className="py-2" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                                            {serviceNames.map(service => (
                                                <a key={service} href="/" onClick={() => setServicesOpen(false)} className="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-sky-500" role="menuitem">
                                                    {service}
                                                </a>
                                            ))}
                                        </div>
